...
public class StackManager
{
          ...
          // Semaphore declarations. Insert your code in the following:
           ...
           private static Semaphore empty = new Semaphore(CharStack.MAX_SIZE);
                 ...
                 public void run()
                 {
                              ...
                              for (int i = 0; i < StackManager.iThreadSteps; i++) {
                            	    try {
                            	        ...
                            	        System.out.println("Consumer thread [TID=" + this.iTID + "] stack is empty.");
                            	    }
                            	}
                     ...
                     public void run()
                     {
                                ...
                                for (int i = 0; i < StackManager.iThreadSteps; i++) {
                                    try {
                                        ...
                                        mutex.Signal();
                                        full.Signal();
                                    } catch (CharStackEmptyException e) { //to ensure stack.pick() works
                                        this.block = 'a';  // if the stack is empty, start with 'a'
                                        try {
                               ...
                               for (int i = 0; i < 2 * StackManager.iThreadSteps; i++) {
                            	   ...
                            	   try {
                                       ...
                                       for (int j = 0; j < stack.getSize(); j++) {
                                    	   if (j==0) {System.out.print("[" + stack.getAt(j) + "]");}
                                    	   else {System.out.print(",[" + stack.getAt(j) + "]");}
                                       }
                                       System.out.println(")");
